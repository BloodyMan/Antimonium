...
/mob/proc/HandleImpact(var/mob/attacker, var/obj/item/hitby, var/meters_per_second)
	...
	if(CheckCoverage(limb.limb_id))
		for(var/thing in coverage_by_bodypart[limb.limb_id])
			...
			var/penetration = force/(clothes.material.GetTensileStrength() * (hitby.GetContactArea() * 0.01))
	...
	if(force)
		...
		NotifyNearby("Wound would be [penetration]cm deep.")


	//var/wound_type = WOUND_CUT
	//if(!penetration) wound_type = WOUND_BRUISE

	//limb.HandleAttacked(attacker, hitby, force, wound_type)
