...
/obj/item/limb/proc/HandleAttacked(var/mob/attacker, var/obj/item/hitby, var/force, var/wound_type)

	if(!owner) return

	var/attack_contact_size = hitby.GetContactArea()
	if(wound_type == WOUND_CUT)
		Splatter(get_turf(owner), owner.blood_material)
		cumulative_wound_depth += force
		cumulative_wound_size += attack_contact_size
	cumulative_overall_damage += max(attack_contact_size, force)

	ExpandWoundOfType(wound_type, force, attack_contact_size, hitby)
	...
	if(organs.len && (cumulative_wound_size >= attack_contact_size || cumulative_overall_damage >= GetContactArea()))
		...
		var/damaging = max(1,rand(round(force * 0.5), round(force * 0.75)))
